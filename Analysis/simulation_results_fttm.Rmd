...
plot_Brier %>%
  filter(beta_type == "simple") %>%
  ggplot(aes(est_model, log_value, fill = est_model)) + 
  geom_boxplot(outlier.shape = NA, outlier.size = 0.1) +
  scale_fill_manual(values = my_colors) +
  facet_grid(~family) +
  labs(y = "log out-of-sample Brier score", fill = "model") +
  theme(axis.title.x = element_blank(), axis.text.x = element_blank(), legend.position = "bottom")
...
plot_mise_surv <- table_mise_surv %>%
  ...
  theme(strip.text = element_blank(),
        axis.title.x = element_blank(), axis.text.x = element_blank(), legend.position = "bottom")

plot_mise_coef / plot_mise_surv +
