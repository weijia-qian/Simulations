...
source(here("Source", "fit_penalized_aft_old.R"))
...
sim_data <- simulate_AFT(family = family, n = n, nS =nS, beta_type = beta_type, beta_0 = beta0_true,
                        b = b_true)
#sim_data <- simulate_Cox(n = n, nS = nS, beta_type = beta_type)
...
lambda_list <- c(1000, 1100, 1200, 1300, 1400, 1500)
for (lambda in lambda_list) {
  # Fit the model
  fit <- optimize_AFT(Y, delta, X, data = sim_data$data, family = model, lambda = lambda, se = TRUE)

  # Extract results
  cat("True beta0 =", beta0_true, "Estimated beta0 =", round(fit$beta0_hat, 3), "\n")
  cat("True b =", b_true, "Estimated b =", round(fit$b_hat, 3), "\n")
  mise_beta1 <- mean((sim_data$coefficients$beta1 - fit$beta1_hat)^2)
  cat("MISE of beta1 =", round(mise_beta1, 5), "\n")

  # Summary of coefficient function
  df_coef <- data.frame(time = sim_data$coefficients$time,             
                        true_coef = sim_data$coefficients$beta1,                      
