...
df$time_since_use <- ifelse(df$is_user == 1, df$mins_since_consump, NA)
...
fit_mgcv <- function(data, family, sgrid, bs = "ps", k = 30){
  ...
  #df_surv = data2
  L <- kronecker(matrix(1, 1, 1), t(lvec))
  S <- kronecker(matrix(1, 1, 1), t(sgrid))
  df_surv_user <- data.frame(X = I(pupil_means_user),
                      L = I(L),
                      X_L = I(pupil_means_user * L),
                      S = I(S),
                      #logY = I(logY),
  ...
  return(list(fit, surv_user, surv_nonuser))
}
```
...
fit1.logistic = pfr(is_user ~ age_in_years + bmi + lf(X_mat, argvals = sgrid), 
...
cindex_laft <- cindex_aaft <- cindex_lfcm <- cindex_afcm <- rep(0, n_folds)
...
df_cindex <- data.frame(Model = c("Linear AFT", "Additive AFT", "LFCM", "AFCM"),
                        C_index = c(mean(cindex_laft), mean(cindex_aaft), mean(cindex_lfcm), mean(cindex_afcm)))
