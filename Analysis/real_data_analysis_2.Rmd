...
# time grid
sgrid <- unique(df$time) # remove the last frame
...
fit1.laft <- fit_mgcv(data = df_post2_r, family = "LAFT", sgrid = sgrid)

# linear functional Cox model
fit1.lfcm <- fit_mgcv(data = df_post2_r, family = "LFCM", sgrid = sgrid)

# lognormal lfAFT
fit1.faft <- fit_mgcv(data = df_post2_r, family = "lognormal", sgrid = sgrid) 

# loglogistic lfAFT
fit1.faft2 <- fit_mgcv(data = df_post2_r, family = "loglogistic", sgrid = sgrid)
```
...
coef.est.laft <- predict(fit1.laft[[1]], newdata = df_pred, type = "terms", term = "s(S):X_L", se.fit = TRUE)
coef.est.lfcm <- predict(fit1.lfcm[[1]], newdata = df_pred, type = "terms", term = "s(S):X_L", se.fit = TRUE)

plot_coef <- data.frame(time = sgrid,
                        coef_faft2 = fit1.faft2[[1]]$beta1_hat,
                        coef_faft2_lb = fit1.faft2[[1]]$beta1_ci_lower,
                        coef_faft2_ub = fit1.faft2[[1]]$beta1_ci_upper,
...
i=1
  ...
  p_user <- ggplot(data = plot_surv, aes(x = time)) +
    # geom_line(aes(y = cdf_faft, color = "lognormal lfAFT"), size = 1) +
    geom_line(aes(y = cdf_faft2, color = "loglogistic lfAFT"), size = 1) +
    geom_line(aes(y = cdf_laft, color = "lognormal lfAFT"), size = 1) +
    geom_line(aes(y = cdf_aaft, color = "afAFT"), size = 1) +
    geom_line(aes(y = cdf_lfcm, color = "LFCM"), size = 1) +
    geom_line(aes(y = cdf_afcm, color = "AFCM"), size = 1) +
    scale_color_manual(values = my_colors,
                       breaks = c("loglogistic lfAFT", "lognormal lfAFT", "afAFT", "LFCM", "AFCM")) +
    labs(
      x = "Time since smoking (minutes)",
      y = expression("P(T"<="t)"),
...
tgrid <- seq(0, 120, len = 1000) 
  ...
  p_nonuser <- ggplot(data = plot_surv, aes(x = time)) +
    # geom_line(aes(y = cdf_faft, color = "lognormal lfAFT"), size = 1) +
    geom_line(aes(y = cdf_faft2, color = "loglogistic lfAFT"), size = 1) +
    geom_line(aes(y = cdf_laft, color = "lognormal lfAFT"), size = 1) +
    geom_line(aes(y = cdf_aaft, color = "afAFT"), size = 1) +
...
for(k in 1:n_folds){
    ...
    eta_logistic <- predict(fit1.logistic, newdata = df_post2_r, type = "response")
...
df_Brier <- data.frame(Model = c("loglogistic lfAFT", "lognormal lfAFT", "mgcv lfAFT", "mgcv afAFT", "mgcv LFCM", "mgcv AFCM", "Functional Logistic"),
                       Brier_score = c(mean(Brier_faft2), mean(Brier_faft), mean(Brier_laft), mean(Brier_aaft), 
                                       mean(Brier_lfcm), mean(Brier_afcm), mean(Brier_logistic)))
print(df_Brier)
```
