...
folder_path <- "/Users/weijia/Research/FDA/Simulations/Output/20250630"
...
table_cindex <- all_info %>%
  ...
  summarise(
    ...
    AUC_faft2 = round(mean(AUC_faft2, na.rm = TRUE), 3),
    .groups = "drop"
  )

# Create summary table
table_cindex_1 <- create_table(table_cindex, var = c("AUC_norm", "AUC_cox", "AUC_faft", "AUC_faft2"), beta_type = "simple",
...
table_Brier <- all_info %>%
  ...
  summarise(
    ...
    Brier_faft2 = round(mean(Brier_faft, na.rm = TRUE), 3),
    .groups = "drop"
  )
...
plot_Brier <- table_Brier %>%
  pivot_longer(cols = c("Brier_norm", "Brier_cox", "Brier_faft", "Brier_faft2"),
               names_to = "type",
               values_to = "value") %>%
  mutate(est_model = case_when(str_detect(type, "cox") ~ "LFCM",
                               str_detect(type, "norm") ~ "mgcv lfAFT",
...
table_mise_coef <- all_coef %>%
  ...
  summarise(
    ...
    mise_coef_faft2 = round(mean(mise_coef_faft, na.rm = TRUE), 3),
...
plot_mise_coef <- table_mise_coef %>%
  pivot_longer(cols = c("mise_coef_norm", "mise_coef_faft", "mise_coef_faft2"),
               names_to = "type",
               values_to = "value") %>%
  mutate(est_model = case_when(str_detect(type, "cox") ~ "LFCM",
                               str_detect(type, "norm") ~ "mgcv lfAFT",
...
plot_surv <- table_mise_surv %>%
  pivot_longer(cols = c("mise_surv_norm", "mise_surv_cox", "mise_surv_faft", "mise_surv_faft2"),
               names_to = "type",
               values_to = "value") %>%
  mutate(est_model = case_when(str_detect(type, "cox") ~ "LFCM",
                               str_detect(type, "norm") ~ "mgcv lfAFT",
