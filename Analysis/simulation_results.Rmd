...
folder_path <- "/Users/weijia/Research/FDA/Simulations/Output/20260619"
# folder_path <- "/Users/weijia/Research/FDA/Simulations/Output/20251020"
...
### Average Out-of-Sample C-Index
```{r}
# box plot
table_Cindex <- all_info %>%
  pivot_longer(cols = c("Cindex_faft", "Cindex_cox", "Cindex_faft2"),
               names_to = "type",
               values_to = "value") %>%
  mutate(est_model = case_when(
            str_detect(type, "cox") ~ "LFCM",
...
plot_linear_Cindex <- table_Cindex %>%
  filter(beta_type == "simple") %>%
  ggplot(aes(factor(nS), value, fill = est_model)) + 
  geom_boxplot(outliers = FALSE, fatten = 1.3) +
  scale_fill_manual(values = my_colors) +
  facet_grid(family~n, labeller = labeller(n = label_both, family = label_value), scales = "free") +
  labs(x = "Density of grid (p)", y = "Out-of-sample C-Index", fill = "Model"
       )

ggsave(file.path(fig_dir, "fig_sim_Cindex.jpeg"), plot_linear_Cindex, width = 6, height = 4, dpi = 500)
...
table_mise_coef <- all_coef %>%
  pivot_longer(cols = c("mise_coef_faft", "mise_coef_cox", "mise_coef_faft2"),
               names_to = "type",
               values_to = "value") %>%
...
table_mise_surv <- all_surv %>%
  pivot_longer(cols = c("mise_surv_faft", "mise_surv_cox", "mise_surv_faft2"),
               names_to = "type",
               values_to = "value") %>%
  mutate(est_model = case_when(str_detect(type, "cox") ~ "LFCM",
                               # str_detect(type, "norm") ~ "lognormal lfAFT",
